...
export default function InventoryLocationConfigModal({
  ...
  const validateConfig = (newConfig: LocationConfig): Record<string, string> => {
    ...
    // Validate total quantity <= maximumStock
    const totalQuantity = newConfig.availableQuantity + newConfig.reservedQuantity + newConfig.inTransitQuantity;
    if (newConfig.maximumStock !== undefined && totalQuantity > newConfig.maximumStock) {
      errors.availableQuantity = 'Total quantity (available + reserved + in-transit) cannot exceed maximum stock';
    }
    ...
    // Validate reserved and in-transit quantities are non-negative
    if (newConfig.reservedQuantity < 0) {
      errors.reservedQuantity = 'Reserved quantity cannot be negative';
    }
    if (newConfig.inTransitQuantity < 0) {
      errors.inTransitQuantity = 'In-transit quantity cannot be negative';
    }
  ...
  const totalQuantity = localConfig.availableQuantity + localConfig.reservedQuantity + localConfig.inTransitQuantity;
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <DialogBody>
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  {validationErrors.reservedQuantity && (
                    <p className="text-xs text-red-500 mt-1">{validationErrors.reservedQuantity}</p>
                  )}
                  <p className="text-xs text-muted-foreground mt-1">
                ...
                <div>
                  ...
                  <Input
                    ...
                    disabled={!isEditable}
                    required
                    className={validationErrors.inTransitQuantity ? 'border-red-500' : ''}
                  />
                  {validationErrors.inTransitQuantity && (
                    <p className="text-xs text-red-500 mt-1">{validationErrors.inTransitQuantity}</p>
                  )}
                  <p className="text-xs text-muted-foreground mt-1">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => handleChange('costingMethod', value as 'FIFO' | 'WEIGHTED_AVERAGE' | 'STANDARD_COST')}
                    disabled={!isEditable}
                  >
                ...
                {localConfig.costingMethod === 'STANDARD_COST' && (
                  <div>
                    ...
                    <Input
                      ...
                      required={localConfig.costingMethod === 'STANDARD_COST'}
                    />
            ...
            {/* Pricing */}
            <div className="space-y-4">
              <h3 className="font-semibold text-sm">Purchase Information</h3>
              <div className="grid grid-cols-2 gap-4">
                <div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <p className="text-xs text-muted-foreground ml-6">
                  Item availability is determined by available quantity (no separate checkbox)
                </p>
              </div>
            </div>
